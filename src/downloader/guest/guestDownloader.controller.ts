...
import { GuestDownloaderService } from './guestDownloader.service';
import { CurrentUser } from 'src/common/decorators/current-user.decorator';
import { JwtAuthGuard } from 'src/auth/guards/jwt-auth.guard';

@Controller('guest/downloader')
export class GuestDownloaderController {
    private readonly logger = new Logger(GuestDownloaderController.name);

    constructor(
        private readonly recaptchaService: RecaptchaService,
        private readonly abuseProtection: AbuseProtectionService,
        private readonly eventEmitter: EventEmitter2,
        private readonly downloaderService: GuestDownloaderService
    ) { }
    ...
    async downloadVideo(@Body() dto: CreateTranscriptionDto, @Req() req: Request, @Res() res: Response) {
        ...
        try {
            await this.downloaderService.downloadGuestVideo(dto, res, ip);
        } catch (error) {
